...
WORKDIR /src
COPY ["PhoneBook.csproj", "."]
RUN dotnet restore "./PhoneBook.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./PhoneBook.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Этот этап используется для публикации проекта службы, который будет скопирован на последний этап
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./PhoneBook.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
